...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const promoSection = document.getElementById('promo-section'); // Nova seção Promo
    const announcementsDiv = document.getElementById('store-announcements'); // Seção movida
    const mainElement = document.querySelector('main');
    const footerElement = document.querySelector('footer');

    let produtos = [];
    let carrinho = [];
    let conteudoExtra = []; // Armazena todos os itens (promo e anúncios)
    let duracaoAnimacaoEntrada = 3000;

    // --- Lógica da Animação de Boas-Vindas com Anime.js ---
    function iniciarAnimacaoBoasVindas() { /* ...código animação welcome (sem alterações)... */
    ...
    function iniciarTransicaoParaLoja(delayInicioFadeOut) { /* ...código transição (sem alterações)... */
    ...
    async function carregarConteudoExtra() { /* ...código carregarConteudoExtra (sem alterações)... */
    ...
    // --- Exibir Promoções (Topo) - CORRIGIDO ---
    function exibirPromocoes(promocoesAtivas) {
        ...
        if (promocoesAtivas.length > 0) {
            ...
            promoSection.style.display = ''; // Garante visibilidade da seção

            // Decide se o item é um link
            const linkHref = promo.link_instagram; // Ou outro campo de link
            const Tag = linkHref ? 'a' : 'div'; // Usa <a> se tiver link, senão <div>
            ...
            // Define o HTML interno da seção de promoção
            promoSection.innerHTML = promoHTML;

            // Adiciona a classe visible para o fade-in APÓS o conteúdo ser definido
            requestAnimationFrame(() => { requestAnimationFrame(() => { promoSection.classList.add('visible'); }); });

        } else {
            promoSection.style.display = 'none';
            // Ajusta delays dos elementos seguintes... (lógica sem alterações)
    ...
    async function carregarProdutos() { /* ...código carregar produtos (sem alterações)... */
    ...
    function exibirProdutos() { /* ...código exibir produtos (sem alterações)... */
    ...
    if (btnFinalizarCompra) {
         btnFinalizarCompra.addEventListener('click', () => {
             ...
             let msg=`Olá, Tammy's Store! 👋 Pedido:\n\n`; let total=0;
             ...
             msg+=`*Total: R$ ${total.toFixed(2).replace('.',',')}*\n\nAguardando validação e PIX. 😊`;
             // ★★★ NÚMERO JÁ CONFIGURADO ★★★
             const num = '5511975938366';
             if(num.length < 12 || !/^\d+$/.test(num)){ alert("Erro: Número de WhatsApp configurado inválido."); console.error("Número WhatsApp inválido:", num); return; }
